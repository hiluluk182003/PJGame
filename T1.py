...
def main():
    ...
    time_elapsed = 0  # Thời gian đã trôi qua
    speed_increase_interval = 0.03  # Khoảng thời gian tăng tốc độ (giây)
    speed_increase_amount = 1  # Số đơn vị tốc độ tăng sau mỗi khoảng thời gian

    def start_gameplay():
        nonlocal letters_list
        lanes = [400, 480, 560]  # 3 dòng chữ
        speeds = [1,1,1]  # Tốc độ start cho mỗi dòng chữ
        letters_list = [Letter(lane, speed) for lane, speed in zip(lanes, speeds)]  # Khởi tạo danh sách các chữ cái
        ...
        if show_gameplay:  # Hiển thị gameplay nếu show_gameplay là True
            for letter in letters_list:
                ...
                if len(letters_list) < 3:
                    ...
                    speeds = [1,1,1] # Tốc độ  cho mỗi dòng chữ
                    letters_list.append(Letter(random.choice(lanes), random.choice(speeds)))  # Thêm chữ cái mới
            ...
            time_elapsed += clock.get_rawtime() / 1000  # Lấy thời gian đã trôi qua từ đồng hồ
